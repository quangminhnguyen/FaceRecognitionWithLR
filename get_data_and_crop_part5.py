...
if not os.path.exists("act_test_cropped/"):
    os.makedirs("act_test_cropped/")
    
if not os.path.exists("act_test_uncropped/"):
    os.makedirs("act_test_uncropped/")
...
for a in act_test:
    ...
    for line in open("facescrub_actresses.txt"):
        if a in line:
            ...
            timeout(testfile.retrieve, (line.split()[4], "act_test_uncropped/"+filename), {}, 30)
            if not os.path.isfile("act_test_uncropped/"+filename):
                continue
            ...
            try:
                img = imread('act_test_uncropped/'+filename)
                img = img[y1:y2,x1:x2]
                img = rgb2gray(img)
                img = imresize(img, (32, 32))
                imsave('act_test_cropped/' + filename, img, cmap = cm.gray)
                print(filename)
            except:
